...
async function renderTopSongBox(info, week, month, year) {
  ...
  // --- Update only week/month/year fields, if same song, No need for refresh ---
  if (thisSongId === lastTopSongId) {
    ...
    if (!weekSpan.empty() && weekSpan.text() !== weekText) {
      weekSpan.text(weekText);
    }
    if (!monthSpan.empty() && monthSpan.text() !== monthText) {
      monthSpan.text(monthText);
    }
    if (!yearSpan.empty() && yearSpan.text() !== yearText) {
      yearSpan.text(yearText);
    }
